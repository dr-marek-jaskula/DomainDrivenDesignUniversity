...
public sealed class CachedProductRepository : IProductRepository
{
    ...
    public Task<(IList<TResponse> Responses, int TotalCount)> PageQuery<TResponse>(IFilter<Product>? filter, ISortBy<Product>? sortBy, IPage page, Expression<Func<Product, TResponse>>? select, CancellationToken cancellationToken, params Expression<Func<Product, object>>[] includes)
    {
